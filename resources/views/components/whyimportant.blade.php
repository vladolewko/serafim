<div class="flex flex-col lg:flex-row my-16 lg:my-52 w-full px-4 sm:px-6 xl:w-4/6 lg:w-5/6 lg:mx-auto gap-8 lg:justify-between">
    <div class="w-full lg:w-5/12 order-2 lg:order-1">
        <div class="mb-6 lg:mb-8 flex-1 text-center sm:text-left">
            <p class="text-2xl sm:text-3xl lg:text-[2.5rem] font-bold leading-tight">
                Чому це <span class="text-yellow-400">важливо?</span>
            </p>
            <p class="w-4/6 mx-auto sm:w-full text-slate-600 text-base lg:text-lg mt-2">
            ...
            @foreach($applyings as $key => $value)
                <div class="flex items-center gap-1 sm:gap-2 bg-blue-400 rounded-lg px-1 py-1 sm:px-2 sm:py-2 radio-div">
                    <input id="checkbox-{{ $key }}"
                            class="w-2 h-2 sm:w-4 sm:h-4 cursor-pointer rounded-[3px] sm:rounded-md text-blue-400 bg-white"
                            name="options"
                            type="radio"
                            value="{{ $key }}"
