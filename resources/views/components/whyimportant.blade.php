<div
    class="flex flex-col lg:flex-row my-16 lg:my-32 w-full px-4 sm:px-6 xl:w-4/6 lg:w-11/12 lg:mx-auto gap-8 lg:justify-between">
    <div class="w-full lg:w-5/12 order-2 lg:order-1">
        ...
        <div class="border border-blue-400 rounded-lg p-4 lg:p-6">
            <div class="flex items-center justify-center lg:justify-start gap-3 mb-6 lg:mb-10">
                <img class="w-[30px] h-[30px] lg:w-auto lg:h-auto" src="{{ asset('img/law_knowledge.svg') }}"
                     alt="law_knowledge img">
                <p class="text-slate-600 font-bold text-[10px] lg:text-base" id="top7">
                    ТОП-7 Переваг знання законів та прав людини
                </p>
            </div>

            <ul class="lg:list-disc ml-6 pr-2 mb-6">
                <li class="text-slate-600 text-center lg:text-left text-[12px] sm:text-base leading-tight"
                    id="description">
            ...
            <div class="space-y-1 lg:space-y-3 w-9/12 md:w-[45%] lg:w-full 2xl:w-5/6 mx-auto mb-6">
                <div class="flex gap-3 items-start ">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <span class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-1">Правовий захист у будь-якій ситуації</span>
                </div>
                <div class="flex gap-3 items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-2">Гарантована свобода та
                        недоторканність</p>
                </div>
                <div class="flex gap-3 items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-3">Можливість законного
                        самозахисту</p>
                </div>
                <div class="flex gap-3 items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-4">Контроль над державою,
                        а
                        не навпаки</p>
                </div>
                <div class="flex gap-3 items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-5">Неможливість
                        маніпуляцій</p>
                </div>
                <div class="flex gap-3  items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-6">Готовність до
                        міжнародного захисту</p>
                </div>
                <div class="flex gap-3 items-start">
                    <img class="w-[17px] h-[17px] lg:w-auto lg:h-auto" src="{{ asset('img/check_green.svg') }}"
                         alt="check img">
                    <p class="text-slate-600 text-[11px] md:text-sm lg:text-base" id="argument-7">Повага і
                        впевненість</p>
                </div>
            </div>
        </div>
        ...
        <div class="border border-blue-400 rounded-lg mt-6 lg:mt-10 p-4">
            <div
                class="flex gap-1 lg:gap-3 w-9/12 md:w-10/12 mx-auto lg:mx-0 lg:w-full lg:gap-5 items-center justify-center">
                <img class="w-[25px] h-[25px] lg:w-auto lg:h-auto" src="{{ asset('img/result.svg') }}" alt="result img">
                <div class="text-slate-600 text-[11px] md:text-xs lg:text-base leading-relaxed">
                    <span class="font-bold" id="result">Результат: </span>
                    <span
                        id="result_desc">Ти — не безправний. Ти — свідомий громадянин, який знає, як себе захистити.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="w-full lg:w-6/12 order-1 lg:order-2 lg:gap-[4.1rem] xl:gap-6 flex flex-col">
        ...
        <div class="flex  gap-5 sm:gap-5  lg:gap-10 justify-center sm:justify-center lg:justify-start mt-2 flex-wrap">
            @php
                ...
                $productImages = [];
                $onSaleSoon = [];
                $availableApplyings = []; // Масив тільки доступних категорій
                ...
                foreach($productsForApplying as $product) {
                    ...
                    $onSaleSoon[$applyingKey] = $product->on_sale_soon;

                    // Додаємо до доступних категорій тільки ті, для яких є продукти
                    if (isset($applyings[$applyingKey])) {
                        $availableApplyings[$applyingKey] = $applyings[$applyingKey];
                    }
                ...
                // Отримуємо першу доступну категорію для початкового вибору
                $firstAvailableKey = array_key_first($availableApplyings);
            @endphp
        ...
        <div
            ...
            @php $firstProduct = reset($availableApplyings); $firstKey = key($availableApplyings); reset($availableApplyings); @endphp
            <div id="onSaleSoonSticker" class="absolute right-0 bg-gray-400 rounded-full {{ ($onSaleSoon[$firstKey] ?? 0) == 1 ? '' : 'hidden' }}">
                <img class="rounded-full w-[75px] h-[75px]" src="{{ asset('img/on_sale_soon.png') }}" alt="">
            </div>

            <img id="productImage"
                 src="{{ $productImages[$firstKey] ?? '' }}"
                 alt="Product Image"
        ...
        <div
            ...
            <div class="text-black text-2xl lg:text-4xl">
                <span class="text-yellow-400" id="price">{{ $prices[$firstKey] ?? 600 }}</span> грн
            </div>
            <a href="{{ route('product.show', $productIds[$firstKey] ?? 1) }}" id="productHref"
