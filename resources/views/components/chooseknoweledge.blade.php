                ...
                <div class="hidden duration-500 ease-in-out bg-white" data-carousel-item>
                    <div class="flex lg:gap-10 xl:gap-7 mt-10 justify-center">
                        @foreach($perPage as $item)
                            ...
                            <div class="lg:w-3/12 xl:w-min h-full rounded-xl bg-gradient-to-t from-yellow-400 to-blue-500 p-[2px]">
                                <div class="bg-white flex flex-col items-center rounded-xl">
                                    ...
                                    <div class="relative xl:h-[212px] xl:w-[212px] lg:w-[200px] lg:h-[200px] rounded-xl bg-gray-200 m-3 slef-center overflow-hidden">
                                        ...
                                        {{-- Стікери статусу --}}
                                        @php
                                            $isProduct = !isset($item->reference);
                                            $isInDevelopment = $item->price == 0 && (!$isProduct || $item->on_sale_soon != 1);
                                            $isOnSaleSoon = $isProduct && $item->on_sale_soon == 1;
                                        ...
                                        {{-- Зображення --}}
                                        @if($item->getMedia($imageCollection)->isNotEmpty())
                                            <img class="w-full h-full"
                                                 src="{{ $item->getFirstMediaUrl($imageCollection) }}"
                                                 alt="{{ $item->title ?? $item->name }}">
                                        @endif
                                    </div>

                                    {{-- Інформація про товар/банер --}}
                                    @if($isInDevelopment)
                                        {{-- В розробці --}}
                                    ...
                                    @else
                                        ...
                                        {{-- Ціна (не показуємо для товарів "скоро в продажу") --}}
                                        @if(!$isOnSaleSoon)
                                            <p class="text-3xl font-bold text-center my-4" style="font-weight: 700">
                                                {{ $item->price }} грн
                                            </p>
                                        @endif
                                        ...
                                        {{-- Кнопка переглянути --}}
                                        <a class="flex items-center justify-center bg-yellow-400 w-11/12 text-center m-2 rounded-lg inline-block align-middle h-10 text-black font-bold text-xl hover:bg-yellow-500 transition duration-300 transform hover:scale-105 shadow-md hover:shadow-lg"
                                           href="{{ $isProduct ? route('product.show', $item->id) : $item->reference }}">
                                            переглянути
                                        </a>
                                    @endif
